...
export default function ThemeToggle({ sticky }: { sticky?: boolean }) {
  ...
  return (
    <div className="flex items-center">
      <button
        type="button"
        className={`hs-dark-mode-active:hidden block hs-dark-mode font-medium ${sticky ? 'text-black hover:text-black/75' : 'text-white hover:text-white/75'} ${isDarkMode ? 'hidden' : 'block'}`}
      ...
      <button
        type="button"
        className={`hs-dark-mode-active:block hidden hs-dark-mode font-medium ${sticky ? 'text-black hover:text-black/75' : 'text-white hover:text-white/75'}  ${isDarkMode ? 'block' : 'hidden'}`}
        onClick={handleToggle}
